import Card from 'react-bootstrap/Card';

function TopCard() {
  return (
    <>



    <div className='imagesblck'>
     <img className='img1' src='../src/assets/photos/cv0.png'> 
     </img> <h1 className='ft'>Track Your Fitness, <br></br> day by day
     </h1>

     <h1 className='ft1'>
     <br></br> Best way & idea<br></br>to to fit into shape
     </h1>


     <h1 className='ft2'>
     <br></br> Be Smart Be Productive <br></br> make it for daily routine.
     </h1>

     <img className='img3' src='../src/assets/photos/61QEWIMXGbL._SX522_.jpg'>
        
    </img><h1 className='ft3'>Remind yourself, <br></br> Daily Reminder</h1>

    <img className="img4" src="../src/assets/photos/877332-Gymnastics-Yoga-Pose-Workout.jpg"></img>
     

    </div>
      {/* <Card className='tpcrd'>
        <Card.Img variant="top" src="holder.js/100px180" />
        <Card.Body>
          <Card.Text>
            Some quick example text to build on the card title and make up the
            bulk of the card's content.
          </Card.Text>
        </Card.Body>
      </Card>
      <br />
      <Card className='tpcrd'>
        <Card.Body>
          <Card.Text>
            Some quick example text to build on the card title and make up the
            bulk of the card's content.
          </Card.Text>
        </Card.Body>
        <Card.Img variant="bottom" src="holder.js/100px180" />
      </Card> */}
    </>
  );
}

export default TopCard;